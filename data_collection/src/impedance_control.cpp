...
int main ()
{
    ...
    try {
        ...
        // Set the motion generator with a speed of 0.5 and the goal joint configuration
        // send robot to initial position through the motion generator
        ...
        impedance_control_callback = [&save_data, &model, &position_d, &orientation_d, &stiffness, 
                                        &damping, &null_stiffness, &null_damping, &q_d, &robot](const franka::RobotState& robot_state,
            ...
            //0 command for debugging
            //tau_d = Eigen::VectorXd::Zero(7);

            // Update data to print.
            if (save_data.mutex.try_lock()) {
                save_data.has_data = true;
                save_data.O_T_EE = robot_state.O_T_EE;
                save_data.q = robot_state.q;
                save_data.dq = robot_state.dq;
                save_data.tau_J = robot_state.tau_J;
                save_data.O_F_ext_hat_K= robot_state.O_F_ext_hat_K; 
                save_data.cartesian_contact = robot_state.cartesian_contact; // Assuming this is the contact force
